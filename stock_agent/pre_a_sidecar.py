...
ShadowDecision/StageResult rows. DAILY_REPORT.md is provenance/display only.

The Python validator owns semantic consistency. A syntactically valid model
response is rejected if it omits PRIMARY candidates, promotes an incomplete or
excluded candidate, contradicts its own analysis status, or claims PRE-A_HIGH
while the core trajectory fields are unresolved.
"""
...
from .pre_a_source_v2 import PRE_A_SOURCE_VERSION, PreASourceError, build_pre_a_source_bundle, candidate_index
...
_CANDIDATE_SCHEMA: dict[str, Any] = {
    "type": "object",
    "required": [
        "ticker", "source_grade", "promotion_readiness", "a_trajectory",
        "fundamental_direction", "expectation_gap", "price_lag", "catalyst_window",
        "missing_gates", "promotion_triggers", "demotion_triggers", "expiry_or_recheck",
        "why", "source_limitations",
    ],
    "properties": {
        "ticker": {"type": "string", "minLength": 1, "maxLength": 16},
        "source_grade": {"type": "string", "enum": ["A", "A-", "B+", "B", "EXCLUDE", "NOT_EVALUATED", "UNKNOWN"]},
        "promotion_readiness": {"type": "string", "enum": ["PRE_A_HIGH", "PRE_A", "WATCH_TRAJECTORY", "NONE", "NOT_EVALUATED"]},
        "a_trajectory": {"type": "string", "enum": ["VERY_HIGH", "HIGH", "MEDIUM", "LOW", "NONE", "NOT_EVALUATED"]},
        "fundamental_direction": {"type": "string", "enum": ["STRONG_VERIFIED", "VERIFIED", "PARTIAL_VERIFIED", "WEAK", "NEGATIVE", "UNKNOWN"]},
        "expectation_gap": {"type": "string", "enum": ["VERIFIED", "PARTIAL_VERIFIED", "FORMING", "ABSENT", "UNKNOWN"]},
        "price_lag": {"type": "string", "enum": ["PRESENT", "PARTIAL_VERIFIED", "ABSENT", "UNKNOWN"]},
        "catalyst_window": {"type": "string", "enum": ["IMMEDIATE", "NEAR", "IN_WINDOW", "OUTSIDE", "UNKNOWN"]},
        "missing_gates": {
            "type": "array",
            "items": {
                "type": "object",
        ...
        "promotion_triggers": {"type": "array", "items": {"type": "string", "minLength": 1, "maxLength": 500}},
        "demotion_triggers": {"type": "array", "items": {"type": "string", "minLength": 1, "maxLength": 500}},
        "expiry_or_recheck": {"type": "string", "minLength": 1, "maxLength": 300},
        "why": {"type": "string", "minLength": 1, "maxLength": 1400},
        "source_limitations": {"type": "array", "items": {"type": "string", "minLength": 1, "maxLength": 500}},
    },
    "additionalProperties": False,
}
...
PRE_A_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        "analysis_status": {"type": "string", "enum": ["COMPLETE", "INSUFFICIENT_SOURCE_REPORT"]},
        "candidates": {"type": "array", "items": _CANDIDATE_SCHEMA},
        "global_notes": {"type": "array", "items": {"type": "string", "minLength": 1, "maxLength": 800}},
    },
    "additionalProperties": False,
}
...
def _core_pre_a_semantics(candidate: dict[str, Any], ticker: str, high: bool) -> None:
    fundamental = str(candidate.get("fundamental_direction") or "")
    gap = str(candidate.get("expectation_gap") or "")
    lag = str(candidate.get("price_lag") or "")
    catalyst = str(candidate.get("catalyst_window") or "")
    if fundamental not in {"STRONG_VERIFIED", "VERIFIED", "PARTIAL_VERIFIED"}:
        raise PreASidecarError(f"PRE-A requires verified/partially verified fundamental direction: {ticker}")
    if gap not in {"VERIFIED", "PARTIAL_VERIFIED", "FORMING"}:
        raise PreASidecarError(f"PRE-A requires a non-absent expectation gap: {ticker}")
    if lag not in {"PRESENT", "PARTIAL_VERIFIED"}:
        raise PreASidecarError(f"PRE-A requires price lag to be present or partially verified: {ticker}")
    if catalyst not in {"IMMEDIATE", "NEAR", "IN_WINDOW"}:
        raise PreASidecarError(f"PRE-A requires a 1-8 week catalyst window: {ticker}")
    if high:
        if fundamental not in {"STRONG_VERIFIED", "VERIFIED"}:
            raise PreASidecarError(f"PRE_A_HIGH requires verified fundamental direction: {ticker}")
        if gap not in {"VERIFIED", "PARTIAL_VERIFIED"}:
            raise PreASidecarError(f"PRE_A_HIGH requires verified/partially verified expectation gap: {ticker}")
...
def validate_sidecar_payload(payload: dict[str, Any], source_bundle: dict[str, Any]) -> None:
    ...
    if analysis_status not in {"COMPLETE", "INSUFFICIENT_SOURCE_REPORT"}:
        raise PreASidecarError("PRE-A analysis_status is invalid")
    ...
    for candidate in candidates:
        ...
        trajectory = str(candidate.get("a_trajectory") or "")
        decision = source.get("decision") if isinstance(source.get("decision"), dict) else {}
        operational_failure_stages = (
            "CANDIDATE_ENGINEERING_FAILURE", "RESEARCH_PROVIDER_FAILURE",
            "SEC_PROVIDER_FAILURE", "SEC_STALE_DATA",
        )
        has_operational_failure = any(_stage_result(source, stage) for stage in operational_failure_stages)
        incomplete_primary = has_operational_failure or bool(decision.get("not_evaluated"))

        # A model cannot claim useful trajectory/readiness while simultaneously
        # declaring its source insufficient or while PRIMARY is not evaluated.
        if analysis_status == "INSUFFICIENT_SOURCE_REPORT" or incomplete_primary:
            if readiness not in {"NONE", "NOT_EVALUATED"} or trajectory not in {"NONE", "NOT_EVALUATED"}:
                raise PreASidecarError(f"incomplete PRE-A analysis cannot claim promotion/trajectory readiness: {ticker}")

        # Authoritative terminal states must remain terminal in the sidecar.
        if source_grade == "EXCLUDE":
            if readiness != "NONE" or trajectory != "NONE":
                raise PreASidecarError(f"EXCLUDE cannot carry PRE-A or trajectory readiness: {ticker}")
        elif source_grade in {"UNKNOWN", "NOT_EVALUATED"}:
            if readiness not in {"NONE", "NOT_EVALUATED"} or trajectory not in {"NONE", "NOT_EVALUATED"}:
                raise PreASidecarError(f"uncertified candidate cannot carry PRE-A or trajectory readiness: {ticker}")
        elif source_grade in {"A", "A-"}:
            if readiness in {"PRE_A", "PRE_A_HIGH", "WATCH_TRAJECTORY"}:
                raise PreASidecarError(f"already-certified A/A- candidate must not enter PRE-A trajectory states: {ticker}")
        elif source_grade == "B" and readiness in {"PRE_A", "PRE_A_HIGH"}:
            raise PreASidecarError(f"B grade is not PRE-A eligible: {ticker}")

        if bool(source.get("grade_conflict")) and readiness in {"PRE_A", "PRE_A_HIGH"}:
        ...
        if readiness in {"PRE_A", "PRE_A_HIGH"}:
            ...
            _core_pre_a_semantics(candidate, ticker, readiness == "PRE_A_HIGH")
            ...
            if critical:
                raise PreASidecarError(f"PRE-A cannot contain CRITICAL unresolved gates: {ticker}")
    ...
    # COMPLETE means complete projection, not "whatever subset the model chose".
    # Exact coverage prevents silent false-negative omission of a B+ candidate.
    missing = sorted(set(structured) - seen)
    extra = sorted(seen - set(structured))
    if missing or extra:
        raise PreASidecarError(f"PRE-A candidate coverage mismatch: missing={missing} extra={extra}")


def _render_markdown(
    *, source_report: Path, source_hash: str, structured_source_hash: str,
    payload: dict[str, Any], provider_name: str, model_name: str, telemetry: dict[str, Any],
) -> str:
    lines = [
        ...
        "- broker_write_count: `0`", "- primary_mutation: `NO`",
        "- markdown_semantic_authority: `NO`", "- web/sec/news re-search: `NO`", "", "## Summary", "",
    ]
    ...
    for item in candidates:
        lines += [
            "", f"## {item['ticker']}", "", f"- Source Grade: `{item['source_grade']}`",
            f"- Promotion Readiness: `{item['promotion_readiness']}`", f"- A-Trajectory: `{item['a_trajectory']}`",
            f"- Fundamental Direction: `{item['fundamental_direction']}`", f"- Expectation Gap: `{item['expectation_gap']}`",
            f"- Price Lag: `{item['price_lag']}`", f"- Catalyst Window: `{item['catalyst_window']}`",
            f"- Recheck / Expiry: {item['expiry_or_recheck']}", "", f"**Why:** {item['why']}", "", "### Missing Gates",
        ]
        ...
        lines += ["", "### Promotion Triggers (recertification request only)"]
        for trigger in item["promotion_triggers"] or ["NONE"]:
            lines.append(f"- {trigger}")
        lines += ["", "### Demotion Triggers"]
...
def generate_pre_a_report(
    source_report: Path, output_path: Path, *, source_bundle: dict[str, Any],
    provider: Any, prompt_text: str, reasoning_effort: str,
) -> Path:
    ...
    request = {
        ...
        "messages": [
            ...
            {"role": "user", "content": "STRUCTURED_PRIMARY_SOURCE_BEGIN\n" + model_source + "\nSTRUCTURED_PRIMARY_SOURCE_END"},
        ],
    ...
    rendered = _render_markdown(
        source_report=source_report, source_hash=source_hash, structured_source_hash=structured_hash,
        payload=payload, provider_name=str(getattr(provider, "provider", "unknown")),
        model_name=str(getattr(provider, "model", "unknown")), telemetry=dict(telemetry or {}),
    )
...
def main() -> int:
    ...
    output_path = args.output or project_root / "pre_a_reports" / args.shadow_run_id / "PRE_A_REPORT.md"
    try:
    ...
    if args.llm_provider == "luna":
        ...
        provider = OpenAIResponsesProvider(
            require_secret("OPENAI_API_KEY"), os.getenv("OPENAI_MODEL", "gpt-5.6-luna"),
            reasoning_effort=effort, timeout=float(os.getenv("LUNA_TIMEOUT_SEC", "90")),
        )
    else:
    ...
    target = generate_pre_a_report(
        args.source_report, output_path, source_bundle=source_bundle, provider=provider,
        prompt_text=prompt_text, reasoning_effort=effort,
    )
    print(json.dumps({
        "ok": True, "source_report": str(args.source_report),
        "structured_source_hash": source_bundle.get("source_hash"), "pre_a_report": str(target),
        "authoritative": False, "primary_mutation": False, "broker_write_count": 0,
