...
class DiscoveryBenchmarkProvider(Protocol):
...
class InMemoryDiscoveryMarketDataProvider:
    ...
    def benchmark_bars(self, tickers: list[str], completed_bar_cutoff: str) -> dict[str, list[DailyBar]]:
        # Benchmark ingestion is a separate telemetry path.  It must not look
        # like candidate candle fetching or invalidate bar-cache assertions.
        return {ticker: [bar for bar in self.bars.get(ticker, [])
                         if bar.session_date <= completed_bar_cutoff[:10]]
                for ticker in tickers if ticker in self.bars}
