...
class DiscoveryMarketRegimeEngine:
    ...
    def evaluate_with_benchmark_returns(self, candidates: list[CandidateFeatureSnapshot],
                                        benchmark_returns: dict[str, float | None]) -> dict:
        """Evaluate regime from a dedicated benchmark provider.

        Benchmark instruments are intentionally not part of the executable
        common-stock universe, so ETF exclusion can never erase regime data.
        """
        returns = [value(item, "return_20d_pct") for item in candidates
                   if known_field(item, "return_20d_pct")]
        benchmark_values = [float(item) for item in benchmark_returns.values() if item is not None]
        if not benchmark_values or not returns:
            return {"regime": "UNKNOWN", "confidence": 0,
                    "reasons": ["BENCHMARK_OR_BREADTH_UNKNOWN"],
                    "benchmarks": benchmark_returns,
                    "breadth": {"eligible_count": len(candidates)}}
        positive = sum(item > 0 for item in returns) / len(returns) * 100
        benchmark_median = median(benchmark_values)
        if benchmark_median > 5 and positive >= 60:
            regime = "BROAD_RISK_ON"
        elif benchmark_median > 0 and positive >= 35:
            regime = "SELECTIVE_RISK_ON"
            ...
            regime = "NEUTRAL"
        return {"regime": regime, "confidence": round(min(100, len(benchmark_values) / 3 * 60 + 40)),
                "reasons": [], "benchmarks": benchmark_returns,
                "breadth": {"eligible_count": len(candidates),
                             "pct_positive_20d": round(positive, 4),
                             "median_return_20d": round(median(returns), 4)}}
