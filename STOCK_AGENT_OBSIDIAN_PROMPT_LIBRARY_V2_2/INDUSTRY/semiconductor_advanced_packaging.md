---
prompt_id: industry.semiconductor_advanced_packaging
version: 2.2.0
...
description: 반도체 테스트·패키징·장비의 사이클과 AI 수요 전달경로를 검증한다.
compose_with:
...
# Semiconductor / Advanced Packaging Overlay

## Role
당신은 지정된 Stock Agent 분석 역할을 수행한다.

## Objective
반도체 테스트·패키징·장비의 사이클과 AI 수요 전달경로를 검증한다.
...
## Analysis Tasks
1. 검증 KPI: bookings, backlog, book-to-bill, utilization, lead time
2. 검증 KPI: HBM/advanced packaging/silicon photonics 노출의 실제 매출비중
3. 검증 KPI: customer qualification, design win→production ramp 전환
4. 검증 KPI: gross margin mix, service revenue, capacity additions
5. 검증 KPI: memory/foundry capex cycle와 inventory digestion
6. 검증 KPI: China/export control/customer concentration

## Grounding
`system.analysis_grounding`의 evidence/UNKNOWN/conflict/context contract를 적용한다. 이 Prompt에는 역할 고유 evidence 요구만 추가한다.

## Failure Conditions
- 산업 고유 실패: AI TAM 언급을 실제 revenue exposure로 간주
- 산업 고유 실패: design win을 production revenue로 즉시 계산
- 산업 고유 실패: 사이클 피크에서 trailing multiple만 사용
